...
# Generates videos for 160 prompts from prompts.json
...
# Output directory
OUTPUT_DIR="$PROJECT_ROOT/evaluation/phygenbench/outputs"
...
CUDA_VISIBLE_DEVICES=$GPU_ID python "$PROJECT_ROOT/evaluation/phygenbench/generate_phygenbench_videos.py" \
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "Output directory: $OUTPUT_DIR"
    echo ""
    VIDEO_COUNT=$(ls -1 "$OUTPUT_DIR"/video_output_*.mp4 2>/dev/null | wc -l)
    echo "Generated videos: $VIDEO_COUNT / 160"
    echo ""
    echo "Next steps:"
    echo "1. Submit the generated videos to PhyGenBench evaluation server"
    echo "2. Or run local evaluation using PhyGenBench toolkit"
    echo "════════════════════════════════════════════════════════════════════════════════"
    exit 0
else
