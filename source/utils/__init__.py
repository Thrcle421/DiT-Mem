    ...
    def download_if_necessary(self, use_usp=False):
        if self.path is None:
            ...
            if snapshot_download is None:
                raise RuntimeError("Model download is unavailable in this lightweight build. Please provide `path` directly.")
            ...
            if self.local_model_path is None:
                default_dir = (Path(__file__).resolve().parents[2] / "weights").resolve()
                default_dir.mkdir(parents=True, exist_ok=True)
                self.local_model_path = str(default_dir)
