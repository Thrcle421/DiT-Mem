    ...
    def load(self, file_path="", state_dict={}, device="cuda", torch_dtype=torch.float16, **kwargs):
        ...
        for architecture in architectures:
            if architecture not in self.architecture_dict:
                print(f"    Skip unsupported architecture in config: {architecture}")
                continue
...
class ModelManager:
    def __init__(
        ...
        self.model_name = []
        if model_id_list:
            print("Warning: model downloads are not supported in this simplified build. Ignoring model_id_list.")
        downloaded_files = []
