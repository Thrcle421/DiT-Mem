...
def download_from_modelscope(model_id, origin_file_path, local_dir):
    if snapshot_download is None:
        raise RuntimeError("modelscope is required to download models but is not installed.")
...
def download_from_huggingface(model_id, origin_file_path, local_dir):
    if hf_hub_download is None:
        raise RuntimeError("huggingface_hub is required to download models but is not installed.")
    os.makedirs(local_dir, exist_ok=True)
    file_name = os.path.basename(origin_file_path)
    if file_name in os.listdir(local_dir):
        print(f"    {file_name} has been already in {local_dir}.")
