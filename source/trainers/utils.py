...
def safe_unwrap_model(accelerator, model):
    """
    Retrieve the underlying model without triggering torch.compile() KeyErrors.

    Accelerate can fail with KeyError('_orig_mod') when torch.compile wraps the
    module. In that case we fallback to the plain `.module` chain.
    """
    manual_candidate = model
    seen = set()
    while hasattr(manual_candidate, "module"):
        next_candidate = getattr(manual_candidate, "module")
        if next_candidate is manual_candidate or id(next_candidate) in seen:
            break
        seen.add(id(manual_candidate))
        manual_candidate = next_candidate
    ...
    try:
        ...
        if exc.args and exc.args[0] == "_orig_mod":
            return manual_candidate
        raise
...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def export_trainable_state_dict(self, state_dict, remove_prefix=None):
        ...
        # Special handling: include ALL dit_mem parameters (even frozen ones)
        # This ensures frozen FFT weights are saved with the checkpoint
        dit_mem_param_names = set()
        if hasattr(self, 'pipe') and hasattr(self.pipe, 'dit_mem') and self.pipe.dit_mem is not None:
            for name, param in self.named_parameters():
                if 'pipe.dit_mem' in name:
                    dit_mem_param_names.add(name)
        
        # Combine trainable params with ALL dit_mem params
        params_to_save = trainable_param_names | dit_mem_param_names
        
        state_dict = {name: param for name, param in state_dict.items() if name in params_to_save}
    ...
    def parse_model_configs(self, model_paths, model_id_with_origin_paths, enable_fp8_training=False):
        ...
        if model_paths is not None:
            ...
            if isinstance(model_paths, str):
                model_paths_str = model_paths.strip()
                if model_paths_str:
                    try:
                        loaded_paths = json.loads(model_paths_str)
                        if isinstance(loaded_paths, str):
                            parsed_model_paths = [loaded_paths]
                        else:
                            parsed_model_paths = list(loaded_paths)
                    except json.JSONDecodeError:
                        parsed_model_paths = [p.strip() for p in model_paths_str.split(",") if p.strip()]
            else:
                parsed_model_paths = list(model_paths)

            model_configs += [ModelConfig(path=path, offload_dtype=offload_dtype) for path in parsed_model_paths]
...
class ModelLogger:
    ...
    def save_model(self, accelerator, model, file_name):
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            # ✅ FIX: Handle torch.compile() wrapper during model saving
            # Strategy: safely unwrap DDP/FSDP while preserving torch.compile()
            unwrapped = safe_unwrap_model(accelerator, model)
            
            # If DiT model is compiled, temporarily unwrap it for state_dict extraction
            dit_was_compiled = False
            original_dit = None
            
            if hasattr(unwrapped, 'pipe') and hasattr(unwrapped.pipe, 'dit'):
                # Check if dit is torch.compile() wrapped
                if hasattr(unwrapped.pipe.dit, '_orig_mod'):
                    dit_was_compiled = True
                    original_dit = unwrapped.pipe.dit
                    # Temporarily replace with original module for state_dict
                    unwrapped.pipe.dit = unwrapped.pipe.dit._orig_mod
            
            try:
                # Get state dict without using accelerator.unwrap_model()
                # This avoids the KeyError with torch.compile()
                ...
                # Restore compiled version if it was removed
                if dit_was_compiled and original_dit is not None:
                    unwrapped.pipe.dit = original_dit
