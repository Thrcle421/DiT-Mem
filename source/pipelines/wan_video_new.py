...
from ..models.wan_video_dit_mem import DiTMem
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.dit_mem: DiTMem = None
        self.vae: WanVideoVAE = None
        self.motion_controller: WanMotionControllerModel = None
        self.vace: VaceWanModel = None
        self.vace2: VaceWanModel = None
        self.animate_adapter: WanAnimateAdapter = None
        self.video_retriever = None  # Will be set from training script
        self.retrieval_k = 5  # Default number of videos to retrieve
        ...
        self.units = [
            ...
            WanVideoUnit_RetrievedVideoEmbedder(),  # Add retrieval unit after prompt embedder
            WanVideoUnit_S2V(),
    ...
    def __call__(
        ...
        # Retrieval
        enable_retrieval: Optional[bool] = True,
        # progress_bar
        ...
        inputs_shared = {
            ...
            # Add prompt to shared inputs so retrieval unit can access it (CFG-independent retrieval)
            "prompt": prompt,
        }
        for unit in self.units:
            inputs_shared, inputs_posi, inputs_nega = self.unit_runner(unit, self, inputs_shared, inputs_posi, inputs_nega)

        # Remove prompt from inputs_shared to avoid conflict with inputs_posi when calling model_fn
        # (prompt is needed in inputs_shared for retrieval units, but model_fn expects it in inputs_posi)
        if "prompt" in inputs_shared:
            inputs_shared = {k: v for k, v in inputs_shared.items() if k != "prompt"}
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
        ...
        for s in seeds:
            noise = pipe.generate_noise(shape, seed=s, rand_device=rand_device)
            if vace_reference_image is not None:
...
class WanVideoUnit_RetrievedVideoEmbedder(PipelineUnit):
    """
    Retrieve similar videos based on prompt and encode them through DiT-Mem for reference.
    
    Retrieves videos from database, encodes to latents (.pt files or VAE-encoded .mp4),
    then processes through DiT-Mem to generate reference tokens.
    
    Note: reference_tokens are shared across both CFG branches (positive and negative).
    """
    def __init__(self):
        super().__init__(
            seperate_cfg=False,  # ✅ Share retrieval across both CFG branches
            input_params=("height", "width", "num_frames", "tiled", "tile_size", "tile_stride", "enable_retrieval", "prompt"),
            onload_model_names=("vae", "dit_mem")
        )
    
    def load_and_preprocess_video(self, video_path, num_frames, height, width):
        """Load video from path and return list of PIL Images."""
        ...
        try:
            ...
            frames = [video_data[int(idx)] for idx in indices]
            return frames
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, tiled, tile_size, tile_stride, enable_retrieval, prompt) -> dict:
        """
        Process retrieval and return reference_tokens to inputs_shared.
        
        Returns reference_tokens shared by both CFG branches. Supports .pt (pre-encoded)
        and .mp4 (VAE-encoded) files, then processes through DiT-Mem.
        """
        # Check if retrieval is enabled and retriever is available
        if not enable_retrieval or pipe.video_retriever is None or pipe.dit_mem is None:
            return {}
        
        # Skip if no prompt provided (edge case)
        ...
        try:
            ...
            # Step 1: Retrieve paths (absolute paths to .pt or .mp4 files)
            video_paths = pipe.video_retriever.retrieve(prompt, k=k)
            logger.info(f"Retrieved {len(video_paths)} sources for prompt: '{prompt}'")
            if not video_paths:
                logger.warning(f"No videos retrieved for prompt: {prompt}")
                return {}
            
            # Step 2: Load/encode latents based on file extension
            # video_paths contains absolute paths to either .pt (latents) or .mp4 (videos)
            ...
            for video_path in video_paths:
                try:
                    if video_path.endswith('.pt'):
                        # Load pre-encoded latent file
                        loaded = torch.load(video_path, map_location='cpu', weights_only=False)
                        
                        # Handle dict format (with 'latent' key)
                        if isinstance(loaded, dict) and 'latent' in loaded:
                            latents = loaded['latent']
                        else:
                            latents = loaded
                        
                        # Ensure correct dimensions [C, T, H, W]
                        if latents.dim() == 4:
                            ...
                            logger.warning(f"Unexpected latent shape from {video_path}: {latents.shape}")
                    
                    elif video_path.endswith('.mp4'):
                        # Load video and encode through VAE
                        frames = self.load_and_preprocess_video(video_path, num_frames, height, width)
                        if frames is None:
                            continue
                        ...
                        # Optional: Save latents to disk for faster subsequent runs
                        # Uncomment below to enable. When .pt file exists, it will be loaded directly.
                        # save_dir = "video_latents"
                        # os.makedirs(save_dir, exist_ok=True)
                        # video_filename = os.path.basename(video_path)
                        # latent_filename = video_filename.replace('.mp4', '.pt')
                        # latent_save_path = os.path.join(save_dir, latent_filename)
                        # if latents.shape == (16, 21, 60, 104):
                        #     torch.save(latents.cpu(), latent_save_path)
                        #     logger.info(f"Saved latents to {latent_save_path}")
                        
                        retrieved_latents_list.append(latents)
                        video_count += 1
                    else:
                        logger.warning(f"Unsupported file type for {video_path}, skipping")
                
                except Exception as e:
                    logger.error(f"Failed to process {video_path}: {e}")
                    continue
            
            logger.info(f"Successfully loaded {len(retrieved_latents_list)} sources "
                      f"({latent_count} from pre-encoded .pt files, {video_count} from .mp4 videos)")
            
            if not retrieved_latents_list:
                logger.warning("No latents could be loaded successfully")
                return {}
            
            # Step 3: Stack all latents (from both pre-encoded and VAE-encoded) and process through DiT-Mem
            # All latents now have the same shape and requires_grad status
            # Stack: [N, C, T, H, W] where N = len(retrieved sources)
            retrieved_latents = torch.stack(retrieved_latents_list, dim=0)
            # Add batch dimension: [1, N, C, T, H, W]
            retrieved_latents = retrieved_latents.unsqueeze(0)
            
            # Process through DiT-Mem to generate reference tokens
            pipe.load_models_to_device(["dit_mem"])
            reference_tokens = pipe.dit_mem(retrieved_latents)
            
            logger.info(f"Generated {reference_tokens.shape[1]} reference tokens from {len(retrieved_latents_list)} sources via DiT-Mem")
            
            return {"reference_tokens": reference_tokens}
            
        except Exception as e:
            logger.error(f"Error in RetrievedVideoEmbedder: {e}")
            import traceback
            traceback.print_exc()
            return {}
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    def __init__(self):
        ...
        # Include reference_tokens for retrieval-augmented generation
        self.concat_tensor_names = ["context", "clip_feature", "y", "reference_latents", "reference_tokens"]
    ...
    def process(self, pipe: WanVideoPipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        for name in self.concat_tensor_names:
            ...
            if tensor_posi is not None and tensor_nega is not None:
                # Both branches have the tensor: concat them
                inputs_shared[name] = torch.concat((tensor_posi, tensor_nega), dim=0)
            elif tensor_shared is not None:
                # Tensor is in shared: duplicate for both branches
                inputs_shared[name] = torch.concat((tensor_shared, tensor_shared), dim=0)
            elif tensor_posi is not None and tensor_nega is None:
                # Only positive has tensor: create zeros for negative (CFG contrast)
                # This is the key fix for retrieval: positive gets reference_tokens, negative gets zeros
                tensor_nega_zeros = torch.zeros_like(tensor_posi)
                inputs_shared[name] = torch.concat((tensor_posi, tensor_nega_zeros), dim=0)
                logger.debug(f"CFG merge: {name} - positive={tensor_posi.shape}, negative=zeros (CFG contrast)")
            elif tensor_posi is None and tensor_nega is not None:
                # Only negative has tensor: create zeros for positive (rare case)
                tensor_posi_zeros = torch.zeros_like(tensor_nega)
                inputs_shared[name] = torch.concat((tensor_posi_zeros, tensor_nega), dim=0)
                logger.debug(f"CFG merge: {name} - positive=zeros, negative={tensor_nega.shape}")
        inputs_posi.clear()
        inputs_nega.clear()
        return inputs_shared, inputs_posi, inputs_nega
...
def model_fn_wan_video(
    ...
    reference_tokens: Optional[torch.Tensor] = None,  # Added: retrieval-based reference tokens
    vace_context = None,
    vace_scale = 1.0,
    audio_embeds: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    # Inject reference tokens from retrieved videos
    # reference_tokens: [1, 100, 1536] (after mean pooling across N retrieved videos)
    # Store in dit for block-level access
    # print('x shape before DiT blocks: {}'.format(x.shape))
    # print('reference_latents:', reference_latents.shape if reference_latents is not None else "None")
    # print('reference_tokens:', reference_tokens.shape if reference_tokens is not None else "None")
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            # print('x shape before block {}: {}'.format(block_id, x.shape))
            # Block
            if use_gradient_checkpointing_offload:
                if reference_tokens is not None:
                    with torch.autograd.graph.save_on_cpu():
                        x = torch.utils.checkpoint.checkpoint(
                            ...
                            x, context, t_mod, freqs, reference_tokens,
                            use_reentrant=False,
                        )
                else:
                    with torch.autograd.graph.save_on_cpu():
                ...
                x = block(x, context, t_mod, freqs, reference_tokens)
